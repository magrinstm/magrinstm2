...
moment.tz.setDefault('America/Bahia').locale('br')
...
const tanggal = moment.tz('America/Bahia').format('DD-MM-YYYY')
/********** END OF UTILS **********/
...
let pendaftar = JSON.parse(fs.readFileSync('./database/group/pendaftar.json'))
    ...
    try {
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
                ...
                await bocchi.sendPtt(from, `./media/tts/BANIDO.mp3`, id)
        ...
        if (isGroupMsg && isAutoStickerOn && isMedia && isImage && !isCmd) {
            ...
            await bocchi.sendImageAsSticker(from, imageBase64)
                .then(() => {
                    console.log(`Figurinha feita em ${processTime(t, moment())} segundos`)
                })
                .catch(async (err) => {
                    console.error(err)
        ...
        // Log
        if (isCmd && !isGroupMsg) console.log(color('[PV]'), color(time, 'red'), color(`${command} [${args.length}]`), 'from', color(pushname))
        if (isCmd && isGroupMsg) console.log(color('[GRUPO]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle))
        ...
        function isReg(obj){
            if (obj === true){
                ...
                return bocchi.reply(from, `.`, id) //if user is not registered
            }
        }
        ...
        switch (command) {
            ...
            case 'wiki':
                ...
                misc.wiki(q)
                    .then(async ({ result, status }) => {
                        if (status !== 200) {
                            return await bocchi.reply(from, 'Não encontrei.', id)
                        } else {
            ...
            case 'tts':
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                if (!q) return await bocchi.reply(from, ind.wrongFormat(), id)
                const speech = q.substring(q.indexOf(' ') + 3)
                const ptt = tts(ar[0])
                try {
            ...
            case 'lembrar': // by Slavyan
            case 'lembrete': // by Slavyan
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                if (!q.includes(',')) return await bocchi.reply(from, ind.wrongFormat(), id)
                ...
                await bocchi.sendTextWithMentions(from, `*「 Lembrete 」*\n\nOk, Avisarei quando chegar à hora! :3\n\n➸ *Mensagem*: ${messRemind}\n➸ *Duração*: ${parsedTime.hours} Horas ${parsedTime.minutes} Minutos ${parsedTime.seconds} Segundos\n➸ *De*: @${sender.id.replace('@c.us', '')}`, id)
                const intervRemind = setInterval(async () => {
                    if (Date.now() > reminder.getReminderTime(sender.id, _reminder)) {
                        await bocchi.sendTextWithMentions(from, `⏰ *「 Lembrar 」* ⏰\n\nChegou à hora! @${sender.id.replace('@c.us', '')}\n\n➸ *Mensagem*: ${reminder.getReminderMsg(sender.id, _reminder)}`)
            ...
            case 'help':
                ...
                if (args[0] === '1') {
                    ...
                    await bocchi.sendText(from, ind.menuRecursos())
                } else if (args[0] === '4') {
                    ...
                    await bocchi.sendText(from, ind.grupomenu())
                } else if (args[0] === '7') {
                    if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                    ...
                    await bocchi.sendText(from, ind.comandos())
                } else if (args[0] === '10') {
                    await bocchi.sendText(from, ind.ajuda1())
                } else if (args[0] === '11') {
                    await bocchi.sendText(from, ind.ajuda2())
                } else if (args[0] === '44') {
            ...
            case 'áudios':
                ...
                if (args[0] === '1') {
                    ...
                    await bocchi.sendPtt(from, `./media/tts/mcpozeputo.mp3`, id)
                } else if (args[0] === '3') {
                    await bocchi.sendPtt(from, `./media/tts/gemidao.mp3`, id)
                } else if (args[0] === '4') {
                    await bocchi.sendPtt(from, `./media/tts/tacaopau.mp3`, id)
                } else if (args[0] === '5') {
                    await bocchi.sendPtt(from, `./media/tts/quedelicia.mp3`, id)
                } else if (args[0] === '6') { 
                    await bocchi.sendPtt(from, `./media/tts/acabou.mp3`, id)
                } else if (args[0] === '7') {
                    await bocchi.sendPtt(from, `./media/tts/aleluia.mp3`, id)
                } else if (args[0] === '8') {
                    await bocchi.sendPtt(from, `./media/tts/apaga.mp3`, id)
                } else if (args[0] === '9') {
                    await bocchi.sendPtt(from, `./media/tts/aplausos.mp3`, id)
                } else if (args[0] === '10') {
                    await bocchi.sendPtt(from, `./media/tts/batiocu.mp3`, id)
                } else if (args[0] === '11') {
                    await bocchi.sendPtt(from, `./media/tts/bichao.mp3`, id)
                } else if (args[0] === '12') {
                   await bocchi.sendPtt(from, `./media/tts/daumasugada.mp3`, id)            
                } else if (args[0] === '13') {
                   await bocchi.sendPtt(from, `./media/tts/calabocacorno.mp3`, id)            
                } else if (args[0] === '14') {
                    await bocchi.sendPtt(from, `./media/tts/cavalo.mp3`, id)
                } else if (args[0] === '15') {
                   await bocchi.sendPtt(from, `./media/tts/calabocamacaco.mp3`, id)                   
                } else if (args[0] === '16') {
                    await bocchi.sendPtt(from, `./media/tts/chupameupinto.mp3`, id)
                } else if (args[0] === '17') {
                    await bocchi.sendPtt(from, `./media/tts/continua.mp3`, id)
                } else if (args[0] === '18') {
                    await bocchi.sendPtt(from, `./media/tts/cricri.mp3`, id)
                } else if (args[0] === '19') {
                    await bocchi.sendPtt(from, `./media/tts/desligaopc.mp3`, id)
                } else if (args[0] === '20') {
                    await bocchi.sendPtt(from, `./media/tts/eeuseila.mp3`, id)
                } else if (args[0] === '21') { 
                    await bocchi.sendPtt(from, `./media/tts/emesmoe.mp3`, id)
                } else if (args[0] === '22') {
                    await bocchi.sendPtt(from, `./media/tts/amigoestouaqui.mp3`, id)
                } else if (args[0] === '23') {
                    await bocchi.sendPtt(from, `./media/tts/fbiopen.mp3`, id)
                } else if (args[0] === '24') {
                    await bocchi.sendPtt(from, `./media/tts/fortnite.mp3`, id)
                } else if (args[0] === '25') {
                    await bocchi.sendPtt(from, `./media/tts/johncena.mp3`, id)
                } else if (args[0] === '26') {
                    await bocchi.sendPtt(from, `./media/tts/nobru.mp3`, id)
                } else if (args[0] === '27') {
                   await bocchi.sendPtt(from, `./media/tts/proerd.mp3`, id)            
                } else if (args[0] === '28') {
                   await bocchi.sendPtt(from, `./media/tts/reidogado.mp3`, id)            
                } else if (args[0] === '29') {
                    await bocchi.sendPtt(from, `./media/tts/vitas.mp3`, id)
                } else if (args[0] === '30') {
                   await bocchi.sendPtt(from, `./media/tts/gemido4.mp3`, id)                     
                } else if (args[0] === '31') {
                   await bocchi.sendPtt(from, `./media/tts/moderadortiraessemlk.mp3`, id)    
                } else if (args[0] === '32') {     
                    await bocchi.sendPtt(from, `./media/tts/saiumaisum.mp3`, id)            
                } else {
                    await bocchi.sendText(from, ind.audios1())
                    await bocchi.sendText(from, ind.audios2())
                }
                break
            case 'rules':
            case 'rule':
            ...
            case 'owner':
               // if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                await bocchi.sendText(from, ind.dono())
            break
            case 'donate':
            case 'doar':
            case 'donasi':     
               // if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)          
                await bocchi.sendText(from, ind.donate())
            break
            case 'comandos':               
            case 'comand':  
            case 'comands':  
            ...
            case 'info':
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                await bocchi.sendText(from, `*Memória Ram*: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${Math.round(os.totalmem / 1024 / 1024)} MB\n*CPU*: ${os.cpus()[0].model}`)
            break
            //case 'listblock':
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
            ...
            case 'asupan':
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await bocchi.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Instagram: https://www.instagram.com/ptl_repost.', id)
            ...
            case 'eu':
                ...
                if (quotedMsg) {
                    ...
                    await bocchi.sendFileFromUrl(from, pfp, `${username}.jpg`, ind.profile(username, status, premi, benet, adm), id)
                    await bocchi.sendPtt(from, `./media/tts/JOGO.mp3`, id)
                }
            break
            case 'hartatahta':
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
            ...
            case 'animada':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        ...
                        await bocchi.sendMp4AsSticker(from, videoBase64, { fps: 17, startTime: `00:00:00.0`, endTime : `00:00:10.0`, loop: 0 })
                            .then(async () => {
                                console.log(`Figurinha feita em ${processTime(t, moment())} segundos`)
                                // await bocchi.sendText(from, ind.ok())
                            })
            ...
            case 'chats':  
                 ...
                 bocchi.sendText(from, `Status:\n- *${loadedMsg}* Quantidade de Mensagens\n- *${groups.length}* Grupos\n- *${chatIds.length - groups.length}* Chats PV\n- *${chatIds.length}* Chats Total`)
            break
            ...
            case 'kick':
                ...
                await bocchi.sendTextWithMentions(from, `Volta pro lobby~\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            ...
            case 'leave':
                ...
                if (!isGroupAdmins) return await bocchi.reply(from, ind.adminOnly(), id)
                await bocchi.sendText(from, 'Adeus~ 👋')
                await bocchi.leaveGroup(groupId)
            break
            case 'everyone': // Thanks to ArugaZ
            case 'all': // Thanks to ArugaZ
            case 'todos': // Thanks to ArugaZ
            case 'todomundo': // Thanks to ArugaZ
                 if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                ...
                if (lastEveryone !== undefined && cd - (Date.now() - lastEveryone) > 0) {
                    ...
                    let txt = '╔══✪〘 Lista dos Cornos 〙✪══\n'
                        for (let i = 0; i < groupMem.length; i++) {
                            txt += '╠➥'
                            txt += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
                        }
                    txt += '╚═〘 *K H A A D A J  B O T* 〙'
            ...
            if (!isGroupMsg) return bocchi.reply(from, '.!', id)
            if (!isGroupAdmins) return bocchi.reply(from, '.!', id)
            if (args.length === 1) return bocchi.reply(from, '.!', id)
            if (args[1].toLowerCase() === 'enable') {
                left.push(chat.id)
                fs.writeFileSync('./lib/database/left.json', JSON.stringify(left))
                bocchi.reply(from, '.!', id)
